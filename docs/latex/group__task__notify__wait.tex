\doxysection{task\+\_\+notify\+\_\+wait}
\hypertarget{group__task__notify__wait}{}\label{group__task__notify__wait}\index{task\_notify\_wait@{task\_notify\_wait}}
task. h 
\begin{DoxyPre}int32\_t task\_notify\_wait( uint32\_t ulBitsToClearOnEntry, uint32\_t ulBitsToClearOnExit, uint32\_t *pulNotificationValue, uint32\_t xTicksToWait );\end{DoxyPre}
...
\hline
{\em x\+Ticks\+To\+Wait} & The maximum amount of time that the task should wait in the Blocked state for a notification to be received, should a notification not already be pending when task\+\_\+notify\+\_\+wait() was called. The task will not consume any processing time while it is in the Blocked state. This is specified in kernel ticks, the macro pd\+MS\+\_\+\+TO\+\_\+\+TICSK( value\+\_\+in\+\_\+ms ) can be used to convert a time specified in milliseconds to a time specified in ticks.\\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
If a notification was received (including notifications that were already pending when task\+\_\+notify\+\_\+wait was called) then pd\+PASS is returned. Otherwise pd\+FAIL is returned.
\end{DoxyReturn}
task. h 
\begin{DoxyPre}void vTaskNotifyGiveFromISR( task\_t xTaskHandle, int32\_t *pxHigherPriorityTaskWoken );
...
task\_notify\_wait() API function.
...
task can be returned from the \doxylink{group__c-rtos_ga2e3ed8f26b8c1edcff493bc583f0a76a}{task\_create()} API function used to create the
task, and the handle of the currently running task can be obtained by calling
\doxylink{group__c-rtos_gad8c64e24feaa08391eb61b62ea2cdbd5}{task\_get\_current()}.
