...
queue. h 
\begin{DoxyPre}
int32\_t queue\_prepend(                                queue\_t  xQueue,                                const void       *pvItemToQueue,                                uint32\_t     xTicksToWait                            );
  \end{DoxyPre}
...
struct AMessage *pxMessage;
   ...
   \{
    ...
    if( queue\_prepend( xQueue1, ( void * ) \&ulVar, ( uint32\_t ) 10 ) != pdPASS )
    \{
   ...
   \{
    ...
    pxMessage = \& xMessage;
    queue\_prepend( xQueue2, ( void * ) \&pxMessage, ( uint32\_t ) 0 );
   \}
...
queue. h 
\begin{DoxyPre}
int32\_t \doxylink{group__apix_ga4519896ea7f4a5d2b3726fd172926a8f}{queue\_append(                                queue\_t  xQueue,                                const void       *pvItemToQueue,                                uint32\_t     xTicksToWait                            )};
  \end{DoxyPre}
...
struct AMessage *pxMessage;
   ...
   \{
    // Send an uint32\_t.  Wait for 10 ticks for space to become
    // available if necessary.
    if( queue\_append( xQueue1, ( void * ) \&ulVar, ( uint32\_t ) 10 ) != pdPASS )
    \{
        // Failed to post the message, even after 10 ticks.
    \}
   \}
   ...
   \{
    ...
    // queue is already full.
    pxMessage = \& xMessage;
    queue\_append( xQueue2, ( void * ) \&pxMessage, ( uint32\_t ) 0 );
   \}
...
queue. h 
\begin{DoxyPre}
int32\_t xQueueSend(                           queue\_t xQueue,                           const void * pvItemToQueue,                           uint32\_t xTicksToWait                      );
  \end{DoxyPre}


This is a macro that calls x\+Queue\+Generic\+Send(). It is included for backward compatibility with versions of Free\+RTOS.\+org that did not include the queue\+\_\+prepend() and \doxylink{group__apix_ga4519896ea7f4a5d2b3726fd172926a8f}{queue\+\_\+append()} macros. It is equivalent to \doxylink{group__apix_ga4519896ea7f4a5d2b3726fd172926a8f}{queue\+\_\+append()}.
...
struct AMessage *pxMessage;
   ...
   \{
    // Send an uint32\_t.  Wait for 10 ticks for space to become
    // available if necessary.
    if( xQueueSend( xQueue1, ( void * ) \&ulVar, ( uint32\_t ) 10 ) != pdPASS )
    \{
        // Failed to post the message, even after 10 ticks.
    \}
   \}
   ...
   \{
    ...
    // queue is already full.
    pxMessage = \& xMessage;
    xQueueSend( xQueue2, ( void * ) \&pxMessage, ( uint32\_t ) 0 );
   \}
...
queue. h 
\begin{DoxyPre}
int32\_t xQueueGenericSend(                                 queue\_t xQueue,                                 const void * pvItemToQueue,                                 uint32\_t xTicksToWait                                 int32\_t xCopyPosition                             );
  \end{DoxyPre}


It is preferred that the macros x\+Queue\+Send(), queue\+\_\+prepend() and \doxylink{group__apix_ga4519896ea7f4a5d2b3726fd172926a8f}{queue\+\_\+append()} are used in place of calling this function directly.
...
struct AMessage *pxMessage;
   ...
   \{
    // Send an uint32\_t.  Wait for 10 ticks for space to become
    // available if necessary.
    if( xQueueGenericSend( xQueue1, ( void * ) \&ulVar, ( uint32\_t ) 10, queueSEND\_TO\_BACK ) != pdPASS )
    \{
        // Failed to post the message, even after 10 ticks.
    \}
   \}
   ...
   \{
    ...
    // queue is already full.
    pxMessage = \& xMessage;
    xQueueGenericSend( xQueue2, ( void * ) \&pxMessage, ( uint32\_t ) 0, queueSEND\_TO\_BACK );
   \}
